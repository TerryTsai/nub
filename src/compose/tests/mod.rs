...
#[test]
fn substitute_bare_var() {
    let env = env_pairs(&[("FOO", "bar")]);
    let out = substitute::substitute("hello $FOO baz", &env).unwrap();
    assert_eq!(out, "hello bar baz");
}

#[test]
fn substitute_braced_var() {
    let env = env_pairs(&[("FOO", "bar")]);
    let out = substitute::substitute("x=${FOO}y", &env).unwrap();
    assert_eq!(out, "x=bary");
}

#[test]
fn substitute_default_when_unset() {
    let env = HashMap::new();
    let out = substitute::substitute("x=${MISSING:-fallback}", &env).unwrap();
    assert_eq!(out, "x=fallback");
}

#[test]
fn substitute_default_uses_value_when_set() {
    let env = env_pairs(&[("FOO", "real")]);
    let out = substitute::substitute("x=${FOO:-fallback}", &env).unwrap();
    assert_eq!(out, "x=real");
}

#[test]
fn substitute_dollar_escape() {
    let env = HashMap::new();
    let out = substitute::substitute("price: $$5", &env).unwrap();
    assert_eq!(out, "price: $5");
}

#[test]
fn substitute_undefined_errors() {
    let env = HashMap::new();
    let err = substitute::substitute("$NOPE", &env).unwrap_err();
