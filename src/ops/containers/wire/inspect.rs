//! Engine wire shape for `GET /containers/{id}/json` (compat). Both
//! engines return the same body. Field set is conservative — we only
//! decode what `proto::ContainerDetail` exposes.
...
pub(in crate::ops::containers) struct RawInspect {
...
impl RawInspect {
    pub(in crate::ops::containers) fn into_detail(self) -> ContainerDetail {
